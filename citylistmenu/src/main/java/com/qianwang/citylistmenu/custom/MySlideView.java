package com.qianwang.citylistmenu.custom;
...
import com.qianwang.citylistmenu.MainActivity;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mTextHeight = mHeight / MainActivity.firstPinYin.size();
                position = y / (mHeight / (MainActivity.firstPinYin.size() + 1));
                invalidate();
                break;
            case MotionEvent.ACTION_MOVE:
                if (isSlide) {
                    backgroupColor = getResources().getColor(R.color.darkgray);
                    mTextHeight = mHeight / MainActivity.firstPinYin.size();
                    position = y / (mHeight / MainActivity.firstPinYin.size() + 1) + 1;
                    invalidate();
                }
                break;
            case MotionEvent.ACTION_UP:
